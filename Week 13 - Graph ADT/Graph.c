...
void shortestPath(Graph *g,int begin,int dest,int cost,int *low){
    int i;
    ...
    while(start != NULL){
        ...
        // printf("Current: %d\n",start->data);
        // show(g);
        shortestPath(g,start->data,dest,cost,low);
        cost -= start->weight;
        start = start->next;
    }
...
void show(Graph *g){
    int i;
    for(i=0;i<g->index;i++){
        printf("%d(%d)[%d] -> ",g->arr[i]->data,g->arr[i]->weight,g->visit[i]);
        Node *tmp = g->arr[i]->next;
        while(tmp != NULL){
            printf("%d(%d) -> ",tmp->data,tmp->weight);
            tmp = tmp->next;
        }
        printf("NULL\n");
    }
